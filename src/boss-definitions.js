...
export const BOSS_002 = {
  ...
  steps: [
    ...
    },
    {
      ...
      tables: ['logs_exportacao', 'usuarios', 'acessos_sistema'],
      expectedColumns: ['nome', 'total_exportacoes', 'exportacoes_ip_externo', 'exportacoes_ip_interno'],
      referenceQuery: `SELECT u.nome, COUNT(l.id) AS total_exportacoes, SUM(CASE WHEN l.ip_origem NOT LIKE '10.0.0.%' THEN 1 ELSE 0 END) AS exportacoes_ip_externo, SUM(CASE WHEN l.ip_origem LIKE '10.0.0.%' THEN 1 ELSE 0 END) AS exportacoes_ip_interno FROM logs_exportacao l INNER JOIN usuarios u ON u.id = l.usuario_id GROUP BY u.nome HAVING COUNT(l.id) >= 1 ORDER BY total_exportacoes DESC;`,
  ...
  steps: [
    {
      ...
      objective: `Atualize stg_clientes.nome para a forma padronizada: TRIM + UPPER na primeira letra e LOWER no restante de cada nome (use a expressão SQL SQLite: TRIM(SUBSTR(nome,1,1)) aplicado — na prática: a expressão de higienização deve deixar nomes como ' José da Silva ' como 'José da silva' — ou seja, aplique TRIM(nome) e converta todas as letras para minúsculo exceto a primeira maiúscula: UPPER(TRIM(SUBSTR(nome,1,1))) || LOWER(TRIM(SUBSTR(nome,2))). Execute o UPDATE.`,
      tables: ['stg_clientes'],
      executionMode: 'ddl',
      referenceQuery: `UPDATE stg_clientes SET nome = UPPER(TRIM(SUBSTR(nome,1,1))) || LOWER(TRIM(SUBSTR(nome,2)));`,
      verificationQuery: `SELECT COUNT(*) AS total FROM stg_clientes WHERE nome = TRIM(nome);`,
      exerciseQuery: `SELECT COUNT(*) AS total FROM stg_clientes WHERE nome = TRIM(nome);`,
      expectedResultQuery: `SELECT COUNT(*) AS total FROM stg_clientes;`,
    ...
    },
    {
      ...
      executionMode: 'ddl',
      referenceQuery: `INSERT INTO dim_tempo (data_completa, ano, mes, dia) SELECT DISTINCT SUBSTR(data_venda,7,4) || '-' || SUBSTR(data_venda,4,2) || '-' || SUBSTR(data_venda,1,2), CAST(SUBSTR(data_venda,7,4) AS INTEGER), CAST(SUBSTR(data_venda,4,2) AS INTEGER), CAST(SUBSTR(data_venda,1,2) AS INTEGER) FROM stg_vendas;`,
      verificationQuery: `SELECT COUNT(*) AS total FROM dim_tempo;`,
      exerciseQuery: `SELECT COUNT(*) AS total FROM dim_tempo;`,
    ...
    },
    {
      id: 'boss-006-3',
      number: 3,
      title: 'As Três Views do Dashboard',
      concept: 'Três views encadeadas',
      briefing: `O dashboard precisa de três views de leitura: (1) vw_vendas_regiao — faturamento por região com JOIN regioes/vendedores/vendas; (2) vw_produtos_ranking — ranking de produtos por quantidade vendida; (3) vw_staging_sujos — clientes do staging ainda com nome diferente do OLTP (JOIN stg_clientes × clientes).`,
      objective: `Crie a view 'vw_vendas_regiao' retornando: regiao (dim_regioes.nome), gerente e faturamento (soma de vendas.valor_total, coluna 'faturamento'), com COALESCE para regiões sem venda. As views vw_produtos_ranking e vw_staging_sujos serão validadas em seguida — foque nesta primeira.`,
      tables: ['dim_regioes', 'dim_vendedores', 'vendas'],
      executionMode: 'create_view',
      viewName: 'vw_vendas_regiao',
      verificationQuery: 'SELECT * FROM vw_vendas_regiao;',
      expectedResultQuery: `SELECT r.nome AS regiao, r.gerente, COALESCE(SUM(v.valor_total), 0) AS faturamento FROM dim_regioes r LEFT JOIN dim_vendedores vd ON vd.nome IN (SELECT dv.nome FROM dim_vendedores dv) LEFT JOIN vendas v ON v.vendedor_id = vd.id GROUP BY r.nome, r.gerente ORDER BY faturamento DESC;`,
    ...
    },
    {
      ...
      exerciseQuery: `SELECT name FROM sqlite_master WHERE type = 'index' AND name = 'idx_fct_regiao';`,
